...
public class RegexUtils {
	...
	/** 车牌正则验证 */
//	public static final String PATTEM = "^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}(?:(?![A-Z]{4})[A-Z0-9]){4}[A-Z0-9挂学警港澳]{1}$";
	public static final String PATTEM = "^([京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领台A-Z]{1}[a-zA-Z](([DF]((?![IO])[a-zA-Z0-9](?![IO]))[0-9]{4})|([0-9]{5}[DF]))|[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领台A-Z]{1}[A-Z]{1}[A-Z0-9]{4}[A-Z0-9挂学警港澳]{1})$";
	public static final String PATTEM_ONLY_GREEN = "[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}(([0-9]{5}[DF])|([DF][A-HJ-NP-Z0-9][0-9]{4}))";
	public static final String PATTEM_NO_ABOUT_GREEN = "^[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领A-Z]{1}[A-Z]{1}(?:(?![A-Z]{4})[A-Z0-9]){4}[A-Z0-9挂学警港澳]{1}$";
	 ...
	 * @return
	 */
	public static boolean isCarPattem(String pattem, boolean isAboutGreen, boolean isOnlyGreen) {
		if (TextUtils.isEmpty(pattem)) {
			return false;
		}
		//只验证绿牌
		if (isOnlyGreen) {
			Pattern pattern = Pattern.compile(PATTEM_ONLY_GREEN);
			Matcher matcher = pattern.matcher(pattem);
			if (!matcher.matches()) {
				return false;
			} else {
				return true;
		...
		//所有车牌，包含绿牌
		if (isAboutGreen) {
			Pattern p = Pattern.compile(PATTEM);
			Matcher m = p.matcher(pattem);
			if (!m.matches()) {
				return false;
			} else {
				return true;
		...
		//除绿牌以外的所有车牌
		if (!isAboutGreen && !isOnlyGreen) {
			Pattern p = Pattern.compile(PATTEM_NO_ABOUT_GREEN);
			Matcher m = p.matcher(pattem);
			if (!m.matches()) {
				return false;
			} else {
	 ...
	 * 验证字符串是否是合法的ip
	 * @param ip 待验证的ip字符串
	 * @return true 是ip，false不是
	 */
	public static boolean isIp(String ip) {
		String ip_regulation = "\\b((?!\\d\\d\\d)\\d+|1\\d\\d|2[0-4]\\d|25[0-5])\\.((?!\\d\\d\\d)\\d+|1\\d\\d|2[0-4]\\d|25[0-5])\\.((?!\\d\\d\\d)\\d+|1\\d\\d|2[0-4]\\d|25[0-5])\\.((?!\\d\\d\\d)\\d+|1\\d\\d|2[0-4]\\d|25[0-5])\\b";

		return ip.matches(ip_regulation);
	}
